...
### 新增功能
1. **数据源集成**: 从ClickHouse读取API数据，写入MySQL
2. **默认合并算法**: 自动识别数字/字母参数类型进行合并
3. **断点续传**: 支持连续运行，启用checkpoint自动续传
4. **应用区分**: 按应用(从URL提取) + HTTP方法 + 规范化URL分组

### 技术架构
| 组件 | 技术 |
|------|------|
| 数据源 | ClickHouse JDBC 0.3.2 |
| 数据目标 | MySQL Connector 8.0.33 |
| 配置 | YAML (SnakeYAML 2.0) |
| JSON处理 | Jackson 2.13.5 |
